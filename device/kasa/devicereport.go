...
func ExtractAllData(device *types.DeviceConfig) {
	connection := openConnection(device.Ip, 9999)
	...
	report, err := buildPeriodicDeviceReport(device.Model, deviceInfoJson,
		lampInfoJson, supportsLampInfo, realTimeJson, supportsEMeter)
	if err == nil {
		fmt.Printf("%#v\n\n", report)
	} else {
		fmt.Printf("Could not build report for '%s: %s': %v", device.Room, device.Name, err)
	}
}
...
func buildPeriodicDeviceReport(
	...
	realTime []byte, supportsEMeter bool) (*PeriodicDeviceReport, error) {

	var report = PeriodicDeviceReport{}

	deviceReport, err := appendDeviceInfo(model, deviceInfo, report)
	if err != nil {
		return deviceReport, err
	}

	if supportsEMeter {
		periodicDeviceReport, err2 := appendEMeterInfo(model, realTime, report)
		if err2 != nil {
			return periodicDeviceReport, err2
		}
	}

	if supportsLampInfo {
		periodicDeviceReport, err2 := appendLampInfo(lampInfo, report)
		if err2 != nil {
			return periodicDeviceReport, err2
		}
	}
...
// region Device Info
func appendDeviceInfo(model types.DeviceType, deviceInfo []byte, report PeriodicDeviceReport) (*PeriodicDeviceReport, error) {
	...
	if int(data["err_code"].(float64)) != 0 {
		return nil, errors.New("call to fetch system info failed")
	}
...
func appendLampInfo(lampInfo []byte, report PeriodicDeviceReport) (*PeriodicDeviceReport, error) {
	...
	if int(data["err_code"].(float64)) != 0 {
		return nil, errors.New("call to fetch lamp info data failed")
	}
...
func appendEMeterInfo(model types.DeviceType, realTime []byte, report PeriodicDeviceReport) (*PeriodicDeviceReport, error) {
	...
	if int(data["err_code"].(float64)) != 0 {
		return nil, errors.New("call to fetch eMeter data failed")
	}
